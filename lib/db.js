...
function edgeConfigId() {
  // EDGE_CONFIG env var format: https://edge-config.vercel.com/ecfg_xxx?token=xxx
...
function edgeConfigToken() {
  // Extract read token from EDGE_CONFIG connection string
  const val = process.env.EDGE_CONFIG || "";
  const match = val.match(/token=([a-z0-9_]+)/);
  return match ? match[1] : null;
}

const hasEdgeConfig = () => edgeConfigId();

// Write to Edge Config via REST API
async function _ecSet(key, value) {
  const configId = edgeConfigId();
  const apiToken = process.env.EDGE_CONFIG_ACCESS_TOKEN=***;
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on("end", () => {
        ...
        else { console.error("EC write error:", res.statusCode, body); resolve(); }
      });
    });
